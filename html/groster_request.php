...
if (!is_array($body) || ($body['request_type'] ?? '') !== 'guild_roster') {
...
if (
    ...
    || !is_array($units_list)
    || count($units_list) < 1
    || count($units_list) > 100
) {
    http_response_code(400);
    echo json_encode([
        'err_code' => 1,
        'err_txt' => 'Invalid request',
    ]);
    exit();
}
...
try {
    $sql = "SELECT name, defId, rarity,
                   CASE WHEN combatType=2 THEN null ELSE gear END AS gear,
                   CASE WHEN combatType=2 THEN null
                        ELSE greatest(0,relic_currentTier-2) END AS relic
            FROM players
            LEFT JOIN roster ON players.allyCode=roster.allyCode
            WHERE guildId=?
              AND defId IN ($placeholders)
            ORDER BY name";
    ...
    error_log('Roster request DB error: ' . $e->getMessage());
